...
import { Globe, TrendingUp, TrendingDown, Clock, Calendar, RefreshCw, Star, Bell, Newspaper } from "lucide-react"
...
export default function MacroPage() {
  ...
  const getImpactColor = (impact: string) => {
    switch (impact) {
      ...
      case "medium": return "bg-terminal-yellow/20 text-terminal-yellow border-terminal-yellow/30"
      ...
      <Tabs defaultValue="markets" className="w-full">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {isLoading && prices.length === 0 ? (
              Array.from({ length: 8 }).map((_, i) => (
                <Card key={i} className="border-terminal-green/20 bg-card/50 animate-pulse">
              ...
              filteredPrices.map((price) => (
                <Card key={price.symbol} className="border-terminal-green/20 bg-card/50 transition-all hover:border-terminal-green/40">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold">{price.symbol}</span>
                        <Badge variant="outline" className="border-terminal-cyan/30 text-terminal-cyan text-xs">
                          {price.category}
                        </Badge>
                      </div>
                    ...
                    <div className="mt-2 flex items-baseline gap-2">
                      <span className="font-mono text-2xl font-bold">
                        {price.price.toLocaleString(undefined, { 
                          ...
                          maximumFractionDigits: price.symbol.includes("BTC") || price.symbol.includes("XAU") ? 2 : 5
                        })}
                      </span>
                    </div>
                    <div className={`mt-1 flex items-center gap-1 font-mono text-sm ${
                      price.change >= 0 ? "text-terminal-green" : "text-terminal-red"
                    }`}>
                      {price.change >= 0 ? (
        ...
        <TabsContent value="calendar" className="mt-6 space-y-6">
          <Card className="border-terminal-green/20 bg-card/50">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="font-mono text-terminal-green">Economic Calendar</CardTitle>
                  <CardDescription>Today&apos;s high-impact events</CardDescription>
                </div>
                <div className="flex gap-2">
                  <Badge variant="outline" className={getImpactColor("high")}>High</Badge>
                  <Badge variant="outline" className={getImpactColor("medium")}>Medium</Badge>
                  <Badge variant="outline" className={getImpactColor("low")}>Low</Badge>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {economicEvents.map((event) => (
                  <div 
                    key={event.id} 
                    className="flex items-center justify-between rounded-lg border border-terminal-green/10 bg-background/50 p-4 transition-colors hover:border-terminal-green/30"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <Badge variant="outline" className="border-terminal-cyan/30 font-mono">
                        {event.currency}
                      </Badge>
                      <div>
                    ...
                    <div className="flex items-center gap-6 text-sm">
                      <div className="text-center">
                        ...
                        <p className="font-mono font-medium text-terminal-cyan">{event.forecast || "-"}</p>
                      </div>
                      <div className="text-center">
                      ...
                      <div className="text-center">
                        ...
                        <p className="font-mono font-medium text-terminal-green">{event.actual || "-"}</p>
                      </div>
        ...
        <TabsContent value="news" className="mt-6 space-y-6">
          <div className="space-y-4">
            {marketNews.map((news) => (
              <Card key={news.id} className="border-terminal-green/20 bg-card/50 cursor-pointer transition-all hover:border-terminal-green/40">
